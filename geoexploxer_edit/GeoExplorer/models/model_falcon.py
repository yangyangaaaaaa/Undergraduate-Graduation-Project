...
class FalconModel(FalconPreTrainedModel):
    def __init__(self, config: FalconConfig):
        ...
        self.action_tokens = nn.Embedding(
            config.num_actions, config.word_embed_proj_dim)
        self.patch_pos_embeds = nn.Linear(4, config.hidden_size, bias=False)
        self.project_action_in = nn.Linear(
            config.word_embed_proj_dim, config.hidden_size, bias=False)
        self.project_out = nn.Linear(config.hidden_size, config.word_embed_proj_dim, bias=False)

        # Final Layer Norm
        self.ln_f = LayerNorm(config.word_embed_proj_dim, eps=config.layer_norm_epsilon)
    ...
    def get_input_tensor(self, input_embeds, actions_embeds):
        inp = torch.zeros((input_embeds.shape[0],
                           input_embeds.shape[1] + actions_embeds.shape[1],
                           input_embeds.shape[-1])).cuda()
        batch = torch.arange(input_embeds.shape[0])
        inp[batch, 0, :] = input_embeds[batch, 0, :]
        inp[batch, 1, :] = input_embeds[batch, 1, :]
        for i in range(actions_embeds.shape[1]):
            inp[batch, 2 * (i + 1), :] = actions_embeds[batch, i, :]
            inp[batch, 2 * (i + 1) + 1, :] = input_embeds[batch, i + 2, :]
        return inp

    def get_action_embeds(self, actions):
        action_list = [[self.action_list[actions[i][j]]
                        for j in range(len(actions[i]))] for i in range(len(actions))]
        actions_list = torch.LongTensor(action_list).cuda()
        return self.action_tokens(actions_list)
    ...
    def forward(
        ...
        patch_sequence: torch.LongTensor = None,
        patch_size: int = None,
        ...
        rel_pos_embeds = self.patch_pos_embeds(self.get_coord_feats(patch_sequence, patch_size))
        inputs_embeds[:, 1:] += rel_pos_embeds
        #inputs_embeds += rel_pos_embeds
        if actions is not None:
            actions_embeds = self.get_action_embeds(actions)
            actions_embeds = self.project_action_in(actions_embeds)
            inputs_embeds = self.get_input_tensor(
                inputs_embeds, actions_embeds)
...
class FalconForCausalLM(FalconPreTrainedModel):
    ...
    def __init__(self, config: FalconConfig):
        ...
        self.distance_pred = nn.Linear(config.word_embed_proj_dim,config.num_actions, bias=False)
        # Initialize weights and apply final processing
        self.post_init()
...
def get_model(name="tiiuae/falcon-7b", num_actions=4, hidden_size=1152):
    from transformers import AutoConfig, AutoTokenizer
    from data_utils import SPECIAL_TOKEN_DICT
    ...
    # print(f"Vocab DIM: {config.word_embed_proj_dim}")
