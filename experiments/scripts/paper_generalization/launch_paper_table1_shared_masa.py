...
LOCAL_SUPERVISOR = SCRIPT_DIR / "paper_table1_shared_masa_supervisor.py"
REMOTE_SUPERVISOR = f"{REMOTE_MONITORING}/paper_table1_shared_masa_supervisor.py"
REMOTE_STATUS = f"{REMOTE_MONITORING}/paper_table1_shared_masa_status_latest.json"
LOCAL_SHARED_BUNDLE = LOCAL_REPO / "analysis" / "pipeline_20260507" / "shared_masa_compare_round1" / "shared_task_bundle.json"
...
def upload_pipeline_files(client: RemoteGeoClient) -> None:
    ...
    if LOCAL_SHARED_BUNDLE.exists():
        client.upload(str(LOCAL_SHARED_BUNDLE), f"{REMOTE_MONITORING}/shared_task_bundle.json")


def compile_scripts(client: RemoteGeoClient) -> None:
    result = client.run("/usr/bin/python3 -m py_compile " + shell_quote(REMOTE_SUPERVISOR))
...
for pid in sorted(p for p in os.listdir('/proc') if p.isdigit()):
    ...
    joined = ' '.join(parts)
    if any(part.endswith('paper_table1_shared_masa_supervisor.py') for part in parts):
...
def launch_supervisor(client: RemoteGeoClient) -> dict:
    ...
    command = (
        ...
        + shell_quote(
            ...
            "nohup /usr/bin/python3 -u paper_table1_shared_masa_supervisor.py "
            "> paper_table1_shared_masa_supervisor.stdout.log 2> paper_table1_shared_masa_supervisor.stderr.log < /dev/null & "
            "echo $! > paper_table1_shared_masa_supervisor.launch.pid; "
            "sleep 2; cat paper_table1_shared_masa_supervisor.launch.pid"
        )
    )
    result = client.run(command)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "failed to launch Table 1 shared-MASA supervisor")
    return {"already_running": False, "supervisor_pid": result.stdout.strip()}


def main() -> int:
    parser = argparse.ArgumentParser(description="Upload and launch paper Table 1 shared-MASA anchor0624 evaluation.")
