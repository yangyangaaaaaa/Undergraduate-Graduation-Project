...
LOCAL_SUPERVISOR = SCRIPT_DIR / "anchor0624_generalization_supervisor.py"
REMOTE_SUPERVISOR = f"{REMOTE_MONITORING}/anchor0624_generalization_supervisor.py"
LOCAL_MANIFEST = EXPERIMENT_ROOT / "comparison_manifest.json"
REMOTE_MANIFEST = f"{REMOTE_EXP_ROOT}/comparison_manifest.json"
REMOTE_STATUS = f"{REMOTE_MONITORING}/anchor0624_generalization_status_latest.json"
...
def upload_pipeline_files(client: RemoteGeoClient) -> None:
    ...
    client.upload(str(LOCAL_MANIFEST), REMOTE_MANIFEST)
    client.upload_dir(str(SCRIPT_DIR), REMOTE_MONITORING)


def compile_supervisor(client: RemoteGeoClient) -> None:
    result = client.run(f"/usr/bin/python3 -m py_compile {shell_quote(REMOTE_SUPERVISOR)}")
...
for pid in sorted(p for p in os.listdir('/proc') if p.isdigit()):
    ...
    joined = ' '.join(parts)
    if any(part.endswith('anchor0624_generalization_supervisor.py') for part in parts):
...
def launch_supervisor(client: RemoteGeoClient) -> dict:
    ...
    command = (
        ...
        + shell_quote(
            ...
            "nohup /usr/bin/python3 -u anchor0624_generalization_supervisor.py "
            "> anchor0624_generalization_supervisor.stdout.log 2> anchor0624_generalization_supervisor.stderr.log < /dev/null & "
            "echo $! > anchor0624_generalization_supervisor.launch.pid; "
            "sleep 2; cat anchor0624_generalization_supervisor.launch.pid"
        )
    )
    result = client.run(command)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "failed to launch anchor0624 generalization eval")
    return {"already_running": False, "supervisor_pid": result.stdout.strip()}


def main() -> int:
    parser = argparse.ArgumentParser(description="Upload and launch anchor0624 factorial generalization evaluation.")
