...
for pid in sorted(p for p in os.listdir('/proc') if p.isdigit()):
    ...
    joined = ' '.join(parts)
    if any(part.endswith('anchor0624_ablation_orchestrator.py') for part in parts):
...
def launch_orchestrator(client: RemoteGeoClient) -> dict:
    ...
    command = (
        ...
        + shell_quote(
            ...
            f"nohup /usr/bin/python3 -u anchor0624_ablation_orchestrator.py "
            f"> anchor0624_ablation_orchestrator.stdout.log 2> anchor0624_ablation_orchestrator.stderr.log < /dev/null & "
            f"echo $! > anchor0624_ablation_orchestrator.launch.pid; "
            f"sleep 2; cat anchor0624_ablation_orchestrator.launch.pid"
        )
    )
    result = client.run(command)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "failed to launch anchor0624 ablation pipeline")
    return {"already_running": False, "orchestrator_pid": result.stdout.strip()}


def main() -> int:
    parser = argparse.ArgumentParser(description="Upload and launch GeoExplorer anchor0624 factorial ablation pipeline.")
    ...
    with RemoteGeoClient(spec) as client:
        ...
        status = client.run(f"cat {shell_quote(REMOTE_MONITORING + '/anchor0624_ablation_status_latest.json')} 2>/dev/null || true")
    print(
        json.dumps(
            {
