...
EXPERIMENT = "anchor0624_dataset_param_appendix_seed321_480k"
TRAIN_EXPERIMENT = "anchor0624_dataset_param_appendix"
...
STATUS_PATH = STATUS_DIR / "appendix_dataset_param_status_latest.json"
PID_PATH = STATUS_DIR / "appendix_dataset_param_orchestrator.pid"

OUTPUT_ROOT = REMOTE_ROOT / "analysis" / "pipeline_20260519_appendix_dataset_param_compare"
...
def build_run_specs() -> list[dict]:
    ...
    dataset_rows = [
        ("dataset_masa_only", "masa_only", "MASA"),
        ("dataset_mmgag_only", "mmgag_only", "MM-GAG"),
        ("dataset_swissview_only", "swissview_only", "SwissView100"),
        ("dataset_masa_plus_mmgag", "masa_plus_mmgag", "MASA+MM-GAG"),
        ("dataset_masa_plus_swissview", "masa_plus_swissview", "MASA+SwissView100"),
        ("dataset_mmgag_plus_swissview", "mmgag_plus_swissview", "MM-GAG+SwissView100"),
        ("dataset_all_three", "all_three", "MASA+MM-GAG+SwissView100"),
    ]
    for prefix, trainset_key, label in dataset_rows:
        roles = [{"table": "dataset", "family": "training_dataset", "value": label}]
        if trainset_key == "masa_plus_mmgag":
            roles.extend(
                [
                    {"table": "param", "family": "gate_floor", "value": "0.405"},
                    {"table": "param", "family": "pbrs_coef", "value": "0.10"},
                    {"table": "param", "family": "ent_coef", "value": "0.005"},
                    {"table": "param", "family": "val_dists", "value": "7,8"},
                    {"table": "param", "family": "train_budget", "value": "480k"},
                    {"table": "seed", "family": "seed", "value": str(DEFAULT_SEED)},
                ]
            )
        specs.append(
            make_spec(
                ...
                trainset_key=trainset_key,
                table_roles=roles,
                factor_values={"training_dataset": label},
                note=f"训练数据集比较：{label}",
            )
        )
    ...
    for value in [0.25, 0.35, 0.50, 0.65]:
        specs.append(
            make_spec(
                f"param_gate_floor_{slug_float(value)}",
                group="param_gate_floor",
                trainset_key="masa_plus_mmgag",
                env_overrides={"GEOEXPLORER_GATE_FLOOR": f"{value:.3f}"},
                factor_values={"gate_floor": value},
                table_roles=[{"table": "param", "family": "gate_floor", "value": f"{value:.3f}"}],
                note="门控下限敏感性",
            )
        )

    for value in [0.0, 0.05, 0.15, 0.20]:
        specs.append(
            make_spec(
                f"param_pbrs_{slug_float(value)}",
                group="param_pbrs",
                trainset_key="masa_plus_mmgag",
                env_overrides={"GEOEXPLORER_PBRS_COEF": f"{value:.2f}"},
                factor_values={"pbrs_coef": value},
                table_roles=[{"table": "param", "family": "pbrs_coef", "value": f"{value:.2f}"}],
                note="PBRS 系数敏感性",
            )
        )

    for value in [0.0025, 0.0075, 0.0100]:
        specs.append(
            make_spec(
                f"param_ent_{slug_float(value)}",
                group="param_entropy",
                trainset_key="masa_plus_mmgag",
                env_overrides={"GEOEXPLORER_ENT_COEF": f"{value:.4f}"},
                factor_values={"ent_coef": value},
                table_roles=[{"table": "param", "family": "ent_coef", "value": f"{value:.4f}"}],
                note="熵系数敏感性",
            )
        )

    for value in ["4,5,6,7,8", "6,7,8"]:
        suffix = value.replace(",", "")
        specs.append(
            make_spec(
                f"param_val_dists_{suffix}",
                group="param_val_dists",
                trainset_key="masa_plus_mmgag",
                env_overrides={"GEOEXPLORER_VAL_DISTS": value},
                factor_values={"val_dists": value},
                table_roles=[{"table": "param", "family": "val_dists", "value": value}],
                note="验证距离选择敏感性",
            )
        )

    for target_steps in [240000, 720000]:
        specs.append(
            make_spec(
                f"param_budget_{target_steps // 1000}k",
                group="param_budget",
                trainset_key="masa_plus_mmgag",
                target_steps=target_steps,
                factor_values={"train_budget": f"{target_steps // 1000}k"},
                table_roles=[{"table": "param", "family": "train_budget", "value": f"{target_steps // 1000}k"}],
                note="训练预算敏感性",
            )
        )
    ...
    for seed in [123, 42]:
        specs.append(
            make_spec(
                f"seed_stability",
                group="seed_stability",
                trainset_key="masa_plus_mmgag",
                seed=seed,
                factor_values={"seed": seed},
                table_roles=[{"table": "seed", "family": "seed", "value": str(seed)}],
                note="最终配置随机种子稳定性",
            )
        )
...
def ensure_train_assets() -> dict[str, dict]:
    ...
    (STAGING_DIR / "appendix_train_assets_manifest.json").write_text(
...
def write_run_repro_bundle(spec: dict, gpu: int | None, train_assets: dict) -> Path:
    ...
    spec_path = out_dir / "appendix_spec.json"
...
def train_all(train_assets: dict) -> list[dict]:
    ...
    while True:
        ...
        write_status(
            "training_appendix_dataset_param",
            {
            ...
            return [
                {
                    **spec,
                    ...
                    "checkpoint_dir": str(ckpt_dir(spec)),
                    "repro_spec": str(ckpt_dir(spec) / "appendix_spec.json"),
                }
                for spec in RUN_SPECS
            ]
        time.sleep(60)
...
def evaluate_all(run_specs: list[dict]) -> dict:
    ...
    while True:
        ...
        write_status(
            "evaluating_appendix_dataset_param",
            {
...
def write_seed_table(rows: list[dict]) -> None:
    seed_rows = family_rows(rows, "seed")
    mean_sr = mean([row["mean_transfer_sr"] for row in seed_rows])
    std_sr = float(math.sqrt(mean([(row["mean_transfer_sr"] - mean_sr) ** 2 for row in seed_rows]))) if seed_rows else math.nan
    with (OUTPUT_ROOT / "appendix_seed_stability_table.csv").open("w", encoding="utf-8", newline="") as handle:
        fieldnames = ["seed", "run", "mean_transfer_sr", "mean_all_sr", "mean_transfer_sg", "mean_all_sg", "checkpoint"]
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for row in sorted(seed_rows, key=lambda item: int(item["value"])):
            writer.writerow(
                {
                    "seed": row["value"],
                    "run": row["run"],
                    "mean_transfer_sr": f"{row['mean_transfer_sr']:.6f}",
                    "mean_all_sr": f"{row['mean_all_sr']:.6f}",
                    "mean_transfer_sg": f"{row['mean_transfer_sg']:.6f}",
                    "mean_all_sg": f"{row['mean_all_sg']:.6f}",
                    "checkpoint": row["checkpoint"],
                }
            )
    return {"mean_transfer_sr_mean": mean_sr, "mean_transfer_sr_std_population": std_sr, "n": len(seed_rows)}
...
def write_markdown_summary(rows: list[dict], seed_stats: dict) -> None:
    dataset_rows = sorted(family_rows(rows, "dataset"), key=lambda item: item["mean_transfer_sr"], reverse=True)
    param_rows = sorted(family_rows(rows, "param"), key=lambda item: (item["family"], -item["mean_transfer_sr"]))
    best_dataset = best_by(dataset_rows, "mean_transfer_sr")
    best_overall = best_by(dataset_rows, "mean_all_sr")
    lines = [
        "# 附录补充实验汇总（自动生成）",
        "",
        f"- 生成时间：`{now_iso()}`",
    ...
    lines.extend(
        [
            "",
            f"- transfer 最优训练数据：`{best_dataset['value'] if best_dataset else 'NA'}`。",
            f"- all-benchmark 最优训练数据：`{best_overall['value'] if best_overall else 'NA'}`。",
            "",
            "## 参数敏感性（按参数族分组）",
            "",
        ]
    ...
    lines.extend(
        [
            "## 随机种子稳定性",
            "",
            f"- `mean_transfer_sr` 平均：`{seed_stats.get('mean_transfer_sr_mean', math.nan):.4f}`",
            f"- `mean_transfer_sr` 标准差（population）：`{seed_stats.get('mean_transfer_sr_std_population', math.nan):.4f}`",
            "",
            "## 写作提醒",
            "",
...
def build_outputs(run_specs: list[dict], benchmarks: dict[str, dict]) -> dict:
    ...
    dataset_rows = sorted(family_rows(summary_rows, "dataset"), key=lambda item: item["mean_transfer_sr"], reverse=True)
    param_rows = sorted(family_rows(summary_rows, "param"), key=lambda item: (item["family"], str(item["value"])))
    seed_stats = write_seed_table(summary_rows)
    write_long_table(long_rows)
    write_wide_table(OUTPUT_ROOT / "appendix_dataset_sr_table.csv", dataset_rows, "sr")
    write_wide_table(OUTPUT_ROOT / "appendix_dataset_sg_table.csv", dataset_rows, "sg")
    write_wide_table(OUTPUT_ROOT / "appendix_param_sr_table.csv", param_rows, "sr")
    write_wide_table(OUTPUT_ROOT / "appendix_param_sg_table.csv", param_rows, "sg")
    write_markdown_summary(summary_rows, seed_stats)
    ...
    aggregate = {
        ...
        "outputs": {
            ...
            "dataset_sr": str(OUTPUT_ROOT / "appendix_dataset_sr_table.csv"),
            "dataset_sg": str(OUTPUT_ROOT / "appendix_dataset_sg_table.csv"),
            "param_sr": str(OUTPUT_ROOT / "appendix_param_sr_table.csv"),
            "param_sg": str(OUTPUT_ROOT / "appendix_param_sg_table.csv"),
            "seed": str(OUTPUT_ROOT / "appendix_seed_stability_table.csv"),
            "summary_zh": str(OUTPUT_ROOT / "appendix_summary_zh.md"),
        },
        "rigor_notes": [
            "All rows are trained and evaluated by the same appendix orchestrator.",
            "Checkpoint selection is validation-based; fixed test banks are not used for model selection.",
            "Dataset comparison includes in-domain and transfer means; transfer means should be emphasized for generalization claims.",
            "Single-run parameter rows diagnose sensitivity; seed-stability rows provide variance for the final configuration.",
        ],
    }
...
def write_pipeline_repro_bundle(train_assets: dict) -> None:
    ...
    (repro_root / "appendix_repro_manifest.json").write_text(
