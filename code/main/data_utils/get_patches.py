...
def get_patches_multiple_images(
    path="../data/swissview/swissview100_patches",
    img_path="../data/swissview/SwissView100/",
    patch_size=5,
):
    path = os.getenv("GEOEXPLORER_PATCH_OUTPUT_DIR", path)
    img_path = os.getenv("GEOEXPLORER_AERIAL_IMAGE_DIR", img_path)
    path = Path(path)
    ...
    for i in range(len(filelist)):
        ...
        get_patches(
            img_path / file,
            save_path=path / f"patches/img_{i}",
