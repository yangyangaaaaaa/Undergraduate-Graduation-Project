#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Build dense MM-GAG C8 checkpoint curves for the 480k-to-960k continuation.
...
INPUT = BISHE_ROOT / "GeoExplorer" / "analysis" / "pipeline_20260606_dense_mmgag_continuation_trend" / "mmgag_checkpoint_eval_all.csv"
OUT_TABLE = TABLES / "dense_mmgag_continuation_eval_real_points.csv"
TREND_TABLE = TABLES / "dense_mmgag_continuation_c8_training_trend_real_points.csv"
SUMMARY_TABLE = TABLES / "dense_mmgag_continuation_c8_method_summary.csv"
FINAL_TABLE = TABLES / "dense_mmgag_continuation_c8_final_checkpoint.csv"
LATE_TABLE = TABLES / "dense_mmgag_continuation_c8_late_stage_summary.csv"
FIGURE = FIGURES / "figure_dense_mmgag_continuation_curves.png"
REPORT = REPORTS / "dense_mmgag_continuation_curve_zh.md"
...
def build_diagnostic_tables(trend: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    final = trend.sort_values("run_progress").groupby("method").tail(1).copy()
    final = final.rename(
        columns={
            "run_progress": "final_progress",
            "time_step": "final_time_step",
            "c8_success": "final_c8_success",
            "worst_modality_sr": "final_worst_modality_sr",
            "c8_residual_distance": "final_c8_residual_distance",
        }
    )
    final = final[
        [
            ...
            "final_time_step",
            "final_c8_success",
            "final_worst_modality_sr",
            "final_c8_residual_distance",
        ]
    ].sort_values("final_c8_success", ascending=False)
    final["final_rank"] = np.arange(1, len(final) + 1)

    late = (
        trend[trend["run_progress"].ge(0.75)]
        .groupby("method")
        .agg(
            late_mean_c8_success=("c8_success", "mean"),
            late_max_c8_success=("c8_success", "max"),
            late_min_c8_success=("c8_success", "min"),
            late_mean_worst_modality_sr=("worst_modality_sr", "mean"),
            late_mean_residual_distance=("c8_residual_distance", "mean"),
            checkpoint_points=("c8_success", "size"),
        )
        .reset_index()
        .sort_values("late_mean_c8_success", ascending=False)
    )
    late["late_mean_rank"] = np.arange(1, len(late) + 1)
    final.to_csv(FINAL_TABLE, index=False, encoding="utf-8-sig")
    late.to_csv(LATE_TABLE, index=False, encoding="utf-8-sig")
    return final, late
...
def build_figure(trend: pd.DataFrame) -> None:
    ...
    fig.text(0.035, 0.955, "480k 续训 Checkpoint 趋势：检验优势是否持续", fontsize=23.5, fontweight="bold", ha="left", va="top")
    fig.text(
        0.035,
        0.918,
        "从 480k 末端 checkpoint 继续训练；圆点为真实保存 checkpoint；奖励机制仅参与训练阶段",
        fontsize=11.5,
        color=MUTED,
        ha="left",
        va="top",
    ...
    draw_method_panel(axes[0], trend, "c8_success_envelope", "C8 成功率 (%)", "平均成功率：续训历史最优", xlim=(0.50, 1.02), ylim=(0, 100))
    draw_method_panel(axes[1], trend, "worst_modality_sr_envelope", "C8 成功率 (%)", "最弱模态成功率：续训历史最优", xlim=(0.50, 1.02), ylim=(0, 100))
    draw_method_panel(axes[2], trend, "c8_closeness_envelope", "接近度 (%)", "目标接近度：续训历史最优", xlim=(0.50, 1.02), ylim=(40, 100))
...
def write_report(summary: pd.DataFrame, final: pd.DataFrame, late: pd.DataFrame) -> None:
    REPORTS.mkdir(parents=True, exist_ok=True)
    best = summary.iloc[0]
    proposed = summary[summary["method"].eq("proposed_linear_gate_pbrs")].iloc[0]
    final_best = final.iloc[0]
    final_proposed = final[final["method"].eq("proposed_linear_gate_pbrs")].iloc[0]
    late_best = late.iloc[0]
    late_proposed = late[late["method"].eq("proposed_linear_gate_pbrs")].iloc[0]
    best_gap = (float(proposed["best_c8_success"]) - float(best["best_c8_success"])) * 100.0
    final_gap = (float(final_proposed["final_c8_success"]) - float(final_best["final_c8_success"])) * 100.0
    late_gap = (float(late_proposed["late_mean_c8_success"]) - float(late_best["late_mean_c8_success"])) * 100.0
    lines = [
        ...
        f"- 历史最佳 checkpoint：`{best['method']}` 最优，C8 mean SR = `{best['best_c8_success']:.4f}`；本文方法为 `{proposed['best_c8_success']:.4f}`，相差 `{best_gap:.2f}` 个百分点。",
        f"- 最终真实 checkpoint：`{final_best['method']}` 最优，C8 mean SR = `{final_best['final_c8_success']:.4f}`；本文方法为 `{final_proposed['final_c8_success']:.4f}`，相差 `{final_gap:.2f}` 个百分点。",
        f"- 后期平均表现（progress >= 0.75）：`{late_best['method']}` 最优，平均 C8 mean SR = `{late_best['late_mean_c8_success']:.4f}`；本文方法为 `{late_proposed['late_mean_c8_success']:.4f}`，相差 `{late_gap:.2f}` 个百分点。",
        "",
        "## 答辩使用判断",
        "",
...
def main() -> dict:
    ...
    final, late = build_diagnostic_tables(trend)
    build_figure(trend)
    write_report(summary, final, late)
    return {
        "input": str(INPUT),
        "figure": str(FIGURE),
