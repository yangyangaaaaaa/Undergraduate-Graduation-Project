...
def generate_config_unseen(data_path, goal_path_list, patch_size=5, n_config_per_img=5, dist=4):
    ...
    for i in range(len(data[()].keys())):
        ...
        for j in range(n_config_per_img):
            CURRENT_PATCH = np.random.randint(0, patch_size**2)
            GOAL_PATCH = goal_path_list[i]
            count = 1
            while (get_dist(CURRENT_PATCH, GOAL_PATCH) != dist or CURRENT_PATCH in patch_list) and count<=1000:
                CURRENT_PATCH = np.random.randint(0, patch_size**2)
                GOAL_PATCH = goal_path_list[i]
                count+=1
            if count>1000:
...
def generate_random_dist_config(data_path, patch_size=5, n_config_per_img=5, dist_possible=[1, 2], sample_mode="balanced"):

    config = defaultdict(list)
    data = np.load(data_path, allow_pickle=True)
    distances = [int(dist) for dist in dist_possible]
    if not distances:
        raise ValueError("dist_possible must contain at least one distance.")

    for i in range(len(data[()].keys())):
        if sample_mode == "balanced":
            episode_dists = [distances[j % len(distances)] for j in range(n_config_per_img)]
        else:
            episode_dists = [int(np.random.choice(distances)) for _ in range(n_config_per_img)]
