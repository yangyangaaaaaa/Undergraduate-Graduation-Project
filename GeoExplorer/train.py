...
if __name__=='__main__':
    ...
    config = generate_random_dist_config(cfg.data.val_path, patch_size=cfg.data.patch_size, dist_possible=[7,8])
    ...
    while time_step <= cfg.train.hparams.max_training_timesteps:
        ...
        for i in range(len(dataset_dict[()].keys())):
            ...
            for t in (range(1, np.random.randint(optimal_steps, cfg.train.hparams.max_ep_len+1))):
                ...
                if inputs["actions"] == []:
                    state, state_preds, state_gt = ppo_agent.llm(inputs_embeds=inputs["inputs_embeds"],
                ...
                reward_ex = ppo_agent.get_reward(cfg.data.patch_size, prev_patch_id, current_patch_id, goal_patch_id, seq.patch_sequence[1:-1], best_dist)
                # ==================inreward cos======================
        ...
        ppo_agent.eval()
        cur_val_success = ppo_agent.validate(config, cfg.data.val_path)[0]
        ...
        if i_episode%50 == 0:
            torch.save(ppo_agent.state_dict(), os.path.join(cfg.train.ckpt_folder, cfg.train.expt_folder, cfg.train.expt_name_tmp)+str(i_episode)+".pt")
